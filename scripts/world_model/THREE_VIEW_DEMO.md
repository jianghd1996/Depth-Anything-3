...
The config uses `12000_lora.safetensors`, eight inference steps, `guidance_scale=0`, 1088×1920 or 1920×1088 output (depending on orientation), and DA3 `process_res=840`. Its prompt path points to `WorldModel-dev/prompt.txt`. CLI options override config values. The checkpoint patch input channels decide whether control-mask conditioning is enabled; DA3 mask previews are always saved.
